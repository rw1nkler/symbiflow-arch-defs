...
class ArchsCollector:
    ...
    def __init__(self, rootdir):
        ...
        """
        assert os.path.exists(rootdir), "Repository root does not exist! {}".format(rootdir)
    ...
    def generate_docs(self, generatedir, searchdirs):
        ...
        if isinstance(searchdirs, list):
            for d in searchdirs:
                ...
                assert os.path.exists(new_dir), "Search directory does not exist! {}".format(searchdirs)
        else:
            new_dir = os.path.join(self.rootdir, d)
            new_searchdirs.append(new_dir)
            assert os.path.exists(new_dir), "Search directory does not exist! {}".format(searchdirs)
    ...
    def _generate_rst_files(self, generatedir):
        ...
        The method uses the Jinja template stored in the
...
class ModelsCollector:
    ...
    def generate_docs(self, generatedir, searchdirs, skip_diagrams=list()):
        ...
        """
        assert isinstance(searchdirs, list), "searchdirs argument should be a list!"
        ...
        if isinstance(searchdirs, list):
            for d in searchdirs:
                new_dir = os.path.join(self.rootdir, d)
                new_searchdirs.append(new_dir)
                assert os.path.exists(new_dir), "Search directory does not exist! {}".format(d)
    ...
    def _update_models(self, file_list, model_type):
        ...
        for f in file_list:
            ...
            if base not in self.models.keys():
                self.models.update({base: {"pb_type": None, "model": None, "sim": None}})
    ...
    def _generate_rst_files(self, generatedir, skip_diagrams):
        ...
        for name, abs_paths in self.models.items():
            ...
            for model_type in ["pb_type", "model", "sim"]:
                if abs_paths[model_type] is not None and os.path.exists(abs_paths[model_type]):
                    model_paths[model_type] = os.path.relpath(abs_paths[model_type], generatedir)
                else:
            ...
            if abs_paths["sim"] is not None:
                with open(abs_paths["sim"]) as sp:
                    data = sp.read()
                    module_name = data.split("module", maxsplit=1)[-1].split(maxsplit=1)[0].split("(")[0]
                ...
                with open(abs_paths["sim"]) as sp:
                    ...
                    for src_line in sp:
                        ...
                        if line.startswith("`include"):
                            ...
                            dep_src = fields[1]
                            dep_path = os.path.realpath(os.path.join(sim_dir, dep_src))
